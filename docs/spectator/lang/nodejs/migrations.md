...
5. If you use meters such as `PercentileDistributionSummary`, `PercentileTimer`, or `DistributionSummary` then you need to 
   update your code to use the respective functions provided by the `Registry` to initialize these meters. The new functions use underscores: `pct_distribution_summary`, `pct_timer`, and `distribution_summary`, respectively.
